...
export function evaluateExpression(expression: LispExpression, symbols: Symbols): string | number | boolean {
    const result = when(expression.op, {
        '+': () => add(expression, symbols),
        '*': () => multiply(expression, symbols),
        '-': () => subtract(expression, symbols),
        '/': () => divide(expression, symbols),
        '^': () => power(expression, symbols),
        '%': () => modulus(expression, symbols),
        '==': () => compare(expression, symbols),
        '>': () => compare(expression, symbols),
        '<': () => compare(expression, symbols),
        '>=': () => compare(expression, symbols),
        '<=': () => compare(expression, symbols),
        'and': () => and(expression, symbols),
        'or': () => or(expression, symbols),
        'not': () => not(expression, symbols),
        'if': () => ifThenElse(expression, symbols),
        'id': () => id(expression),
        '.': () => dot(expression, symbols),
        '++': () => concat(expression, symbols),
        'regex': () => regex(expression, symbols),
        'fake': () => fake(expression)
    })
    return result
}
...
function dot(expression: LispExpression, symbols: Symbols): string | number | boolean {
    if (expression.op === '.' && expression.args.length >= 1 && expression.expectedReturnType !== undefined) {
        const { expectedReturnType } = expression
        // Note. Below line is a dirty hack and should be replaced with an elegant solution in regards to symbol paths generated by expression constructed from query.
        const args = iff(expression.args.length !== 0 && expression.args[0] === 'values' && expression.args[expression.args.length - 1] === 'variableName',
            expression.args.slice(0, expression.args.length - 1), expression.args)
        if (args.length === 1) {
            const symbol = args[0]
            switch (expectedReturnType) {
                case 'Text': {
