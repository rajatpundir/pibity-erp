...
import { DiffRow, MaterialRejectionSlipRow } from '../../../main/rows'
import { MaterialRejectionSlipVariable } from '../../../main/variables'
...
const initialState: State = {
    typeName: 'MaterialRejectionSlip',
    query: getQuery('MaterialRejectionSlip'),
    limit: 5,
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'purchaseInvoice'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.materialRejectionSlips.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<MaterialRejectionSlipVariable>>().appendAll(rows ? rows.map(x => MaterialRejectionSlipRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/material-rejected') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
