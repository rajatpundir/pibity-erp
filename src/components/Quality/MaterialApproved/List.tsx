...
import { MaterialApprovalSlipRow } from '../../../main/rows'
...
const initialState: State = {
    typeName: 'MaterialApprovalSlip',
    query: getQuery('MaterialApprovalSlip'),
    limit: 5,
    offset: 0,
    page: 1,
...
function Component(props) {
    ...
    const queriedVariables = useLiveQuery(() => db.materialApprovalSlips.toArray())
    const variables = (queriedVariables ? queriedVariables.map(x => MaterialApprovalSlipRow.toVariable(x)) : []).filter(variable => applyFilter(state.query, variable))
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/material-approved') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
