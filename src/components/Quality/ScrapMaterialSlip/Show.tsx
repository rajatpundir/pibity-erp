...
function Component(props) {
    ...
    const scrapMaterialSlipType = types['ScrapMaterialSlip']
    ...
    const scrapMaterialSlipRows = useLiveQuery(() => db.ScrapMaterialSlip.toArray())?.map(x => ScrapMaterialSlipRow.toVariable(x))
    var scrapMaterialSlipList = HashSet.of<Immutable<ScrapMaterialSlipVariable>>().addAll(scrapMaterialSlipRows ? scrapMaterialSlipRows : [])
    useLiveQuery(() => db.diffs.toArray())?.map(x => DiffRow.toVariable(x))?.forEach(diff => {
        scrapMaterialSlipList = scrapMaterialSlipList.filter(x => !diff.variables.ScrapMaterialSlip.remove.anyMatch(y => x.id.hashCode() === y.hashCode())).filter(x => !diff.variables.ScrapMaterialSlip.replace.anyMatch(y => y.id.hashCode() === x.id.hashCode())).addAll(diff.variables.ScrapMaterialSlip.replace)
    })
            ...
            return <Container area={none} layout={Grid.layouts.main}>
                <Item area={Grid.header}>
                    <Title>{when(state.mode, {
                        'create': `Create Scrap Material Slip`,
                        'update': `Update Scrap Material Slip`,
                        'show': `Scrap Material Slip`
                    })}</Title>
                </Item>
                <Item area={Grid.button} justify='end' align='center' className='flex'>
                    {
                        iff(state.mode === 'create',
                            <Button onClick={async () => {
                                ...
                                props.history.push('/scrap-material-slip-list')
                            }}>Save</Button>,
                ...
                <Container area={Grid.details} layout={Grid.layouts.details}>
                    <Item>
                        <Label>{scrapMaterialSlipType.keys.productionPreparationSlip}</Label>
                        {
                    ...
                    <Item>
                        <Label>{scrapMaterialSlipType.keys.quantity}</Label>
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
