...
import { DiffRow, TransferMaterialSlipRow } from '../../../main/rows'
import { TransferMaterialSlipVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'productionPreparationSlip'], ['values', 'quantity'], ['values', 'transferred'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.transferMaterialSlips.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<TransferMaterialSlipVariable>>().appendAll(rows ? rows.map(x => TransferMaterialSlipRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/material-transferred') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
