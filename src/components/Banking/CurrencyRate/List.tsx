...
import { DiffRow, CurrencyRateRow } from '../../../main/rows'
import { CurrencyRateVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CurrencyRate.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CurrencyRateVariable>>().appendAll(rows ? rows.map(x => CurrencyRateRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/currency-rate') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
