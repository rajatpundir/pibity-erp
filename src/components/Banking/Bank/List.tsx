...
import { BankRow, DiffRow } from '../../../main/rows'
import { BankVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'country', 'values', 'name'], ['values', 'name'], ['values', 'website'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.banks.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<BankVariable>>().appendAll(rows ? rows.map(x => BankRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                <Title>Banks</Title>
                <button onClick={() => { props.history.push('/bank') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
