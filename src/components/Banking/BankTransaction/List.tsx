...
import { DiffRow, BankTransactionRow } from '../../../main/rows'
import { BankTransactionVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.BankTransaction.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<BankTransactionVariable>>().appendAll(rows ? rows.map(x => BankTransactionRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/bank-transaction') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
