...
import { BankBranchRow, DiffRow } from '../../../main/rows'
import { BankBranchVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'bank', 'values', 'name'], ['values', 'name'], ['values', 'ifsc'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.bankBranches.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<BankBranchVariable>>().appendAll(rows ? rows.map(x => BankBranchRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/bank-branch') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
