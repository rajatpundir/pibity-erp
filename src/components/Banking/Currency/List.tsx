...
import { DiffRow, CurrencyRow } from '../../../main/rows'
import { CurrencyVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.Currency.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CurrencyVariable>>().appendAll(rows ? rows.map(x => CurrencyRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/currency') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
