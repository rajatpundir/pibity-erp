...
import { DiffRow, PurchaseOrderRow } from '../../../main/rows'
import { PurchaseOrderVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'quotation'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.purchaseOrders.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<PurchaseOrderVariable>>().appendAll(rows ? rows.map(x => PurchaseOrderRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/purchase-order') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
