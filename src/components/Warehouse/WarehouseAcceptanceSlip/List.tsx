...
import { DiffRow, WarehouseAcceptanceSlipRow } from '../../../main/rows'
import { WarehouseAcceptanceSlipVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'transferMaterialSlip'], ['values', 'quantity'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.WarehouseAcceptanceSlip.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<WarehouseAcceptanceSlipVariable>>().appendAll(rows ? rows.map(x => WarehouseAcceptanceSlipRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/warehouse-acceptance-slip') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
