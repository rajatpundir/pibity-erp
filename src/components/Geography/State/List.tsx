...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'country', 'values', 'name'], ['values', 'name'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.State.orderBy('id').toArray())
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                <Title>States</Title>
                <button onClick={() => { props.history.push('/state') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
