...
import { DiffRow, PostalCodeRow } from '../../../main/rows'
import { PostalCodeVariable } from '../../../main/variables'
...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'subdistrict', 'values', 'name'], ['values', 'name'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.postalCodes.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<PostalCodeVariable>>().appendAll(rows ? rows.map(x => PostalCodeRow.toVariable(x)) : [])
