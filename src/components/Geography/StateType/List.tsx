...
import { DiffRow, StateTypeRow } from '../../../main/rows'
import { StateTypeVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.StateType.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<StateTypeVariable>>().appendAll(rows ? rows.map(x => StateTypeRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/state-type') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
