...
import { DiffRow, MemoRow } from '../../../main/rows'
import { MemoVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.Memo.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<MemoVariable>>().appendAll(rows ? rows.map(x => MemoRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/memo') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
