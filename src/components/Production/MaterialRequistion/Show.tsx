...
function Component(props) {
        ...
        switch (action[0]) {
            ...
            case 'items': {
                switch (action[1]) {
                    ...
                    case 'addVariable': {
                        state.items.variables = state.items.variables.add(new MaterialRequistionSlipItemVariable(-1, { materialRequistionSlip: new MaterialRequistionSlip(-1), materialApprovalSlipItem: new MaterialApprovalSlipItem(state.items.variable.values.materialApprovalSlipItem.hashCode()), quantity: state.items.variable.values.quantity, consumed: 0 }))
    ...
    const setVariable = useCallback(async () => {
        if (props.match.params[0]) {
            ...
            if (variables.length() === 1) {
                ...
                const itemRows = await db.MaterialRequistionSlipItem.toArray()
                ...
                const items = composedItemVariables.filter(variable => variable.values.materialRequistionSlip.hashCode() === props.match.params[0])
    ...
    const rows = useLiveQuery(() => db.MaterialApprovalSlip.toArray())?.map(x => MaterialApprovalSlipRow.toVariable(x))
    var materialApprovalSlips = HashSet.of<Immutable<MaterialApprovalSlipVariable>>().addAll(rows ? rows : [])
    useLiveQuery(() => db.diffs.toArray())?.map(x => DiffRow.toVariable(x))?.forEach(diff => {
        materialApprovalSlips = materialApprovalSlips.filter(x => !diff.variables.MaterialApprovalSlip.remove.anyMatch(y => x.id.hashCode() === y.hashCode())).filter(x => !diff.variables.MaterialApprovalSlip.replace.anyMatch(y => y.id.hashCode() === x.id.hashCode())).addAll(diff.variables.MaterialApprovalSlip.replace)
    })

    const itemRows = useLiveQuery(() => db.MaterialApprovalSlipItem.where({ materialApprovalSlip: state.variable.values.materialApprovalSlip.hashCode() }).toArray())?.map(x => MaterialApprovalSlipItemRow.toVariable(x))
    var items = HashSet.of<Immutable<MaterialApprovalSlipItemVariable>>().addAll(itemRows ? itemRows : [])
    useLiveQuery(() => db.diffs.toArray())?.map(x => DiffRow.toVariable(x))?.forEach(diff => {
        items = items.filter(x => !diff.variables.MaterialApprovalSlipItem.remove.anyMatch(y => x.id.hashCode() === y.hashCode())).filter(x => !diff.variables.MaterialApprovalSlipItem.replace.anyMatch(y => y.id.hashCode() === x.id.hashCode())).addAll(diff.variables.MaterialApprovalSlipItem.replace)
        items = items.filter(x => x.values.materialApprovalSlip.hashCode() === state.variable.values.materialApprovalSlip.hashCode())
    })
    ...
    const createVariable = async () => {
        const [result, symbolFlag, diff] = await executeCircuit(circuits.createMaterialRequistionSlip, {
            ...
            items: state.items.variables.toArray().map(item => {
                return {
                    materialApprovalSlipItem: item.values.materialApprovalSlipItem.hashCode(),
                        ...
                        <Label>{materialRequistionSlip.keys.materialApprovalSlip.name}</Label>
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
                                <Select onChange={onVariableInputChange} value={state.variable.values.materialApprovalSlip.hashCode()} name='materialApprovalSlip'>
                                    ...
                                    {materialApprovalSlips.toArray().map(x => <option value={x.id.hashCode()}>{x.id.hashCode()}</option>)}
                                </Select>,
                                <div className='font-bold text-xl'>{state.variable.values.materialApprovalSlip.hashCode()}</div>
                            )
                        }
                    </Item>
                </Container>
                            ...
                            <div className='bg-gray-300 font-nunito h-screen overflow-y-scroll' style={{ maxWidth: '90vw' }}>
                                ...
                                <Container area={none} layout={Grid.layouts.uom} className=''>
                                    <Item>
                                        ...
                                        <Select onChange={onItemInputChange} value={state.items.variable.values.materialApprovalSlipItem.hashCode()} name='materialApprovalSlipItem'>
