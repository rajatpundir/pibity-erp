...
import { DiffRow, ProductionPreparationSlipRow } from '../../../main/rows'
import { ProductionPreparationSlipVariable } from '../../../main/variables'
...
const initialState: State = {
    typeName: 'ProductionPreparationSlip',
    query: getQuery('ProductionPreparationSlip'),
    limit: 5,
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'bom'], ['values', 'approved'], ['values', 'scrapped'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ProductionPreparationSlip.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ProductionPreparationSlipVariable>>().appendAll(rows ? rows.map(x => ProductionPreparationSlipRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/production-preparation-slip') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
