...
import { DiffRow, IndentItemRow } from '../../../main/rows'
import { IndentItemVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.IndentItem.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<IndentItemVariable>>().appendAll(rows ? rows.map(x => IndentItemRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/indent-item') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
