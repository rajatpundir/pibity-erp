...
import { DiffRow, ProductCategoryGroupRow } from '../../../main/rows'
import { ProductCategoryGroupVariable } from '../../../main/variables'

type State = Immutable<{
    typeName: 'ProductCategoryGroup'
    query: Query
    limit: number
    offset: number
    page: number
...
const initialState: State = {
    typeName: 'ProductCategoryGroup',
    query: getQuery('ProductCategoryGroup'),
    limit: 5,
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'parent'], ['values', 'name'], ['values', 'length'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ProductCategoryGroup.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ProductCategoryGroupVariable>>().appendAll(rows ? rows.map(x => ProductCategoryGroupRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/product-category-group') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
