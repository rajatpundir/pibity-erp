...
const initialState: State = {
    typeName: 'CompanyTagGroup',
    query: getQuery('CompanyTagGroup'),
    limit: 5,
    offset: 0,
    page: 1,
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyTagGroup.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyTagGroupVariable>>().appendAll(rows ? rows.map(x => CompanyTagGroupRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-tag-group') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
