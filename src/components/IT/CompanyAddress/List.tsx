...
import { DiffRow, CompanyAddressRow } from '../../../main/rows'
import { CompanyAddressVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyAddress.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyAddressVariable>>().appendAll(rows ? rows.map(x => CompanyAddressRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-address') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
