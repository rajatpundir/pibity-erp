...
import { DiffRow, CompanyProductRow } from '../../../main/rows'
import { CompanyProductVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyProduct.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyProductVariable>>().appendAll(rows ? rows.map(x => CompanyProductRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-product') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
