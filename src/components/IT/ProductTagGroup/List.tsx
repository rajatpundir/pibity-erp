...
const initialState: State = {
    typeName: 'ProductTagGroup',
    query: getQuery('ProductTagGroup'),
    limit: 5,
    offset: 0,
    page: 1,
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ProductTagGroup.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ProductTagGroupVariable>>().appendAll(rows ? rows.map(x => ProductTagGroupRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/product-tag-group') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
