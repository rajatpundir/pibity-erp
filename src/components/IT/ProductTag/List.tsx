...
import { DiffRow, ProductTagRow } from '../../../main/rows'
import { ProductTagVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ProductTag.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ProductTagVariable>>().appendAll(rows ? rows.map(x => ProductTagRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/product-tag') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
