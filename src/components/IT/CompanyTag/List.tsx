...
import { DiffRow, CompanyTagRow } from '../../../main/rows'
import { CompanyTagVariable } from '../../../main/variables'
...
const initialState: State = {
    typeName: 'CompanyTag',
    query: getQuery('CompanyTag'),
    limit: 5,
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'group'], ['values', 'name'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyTag.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyTagVariable>>().appendAll(rows ? rows.map(x => CompanyTagRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-tag') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
