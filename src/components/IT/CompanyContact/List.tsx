...
import { DiffRow, CompanyContactRow } from '../../../main/rows'
import { CompanyContactVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyContact.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyContactVariable>>().appendAll(rows ? rows.map(x => CompanyContactRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-contact') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
