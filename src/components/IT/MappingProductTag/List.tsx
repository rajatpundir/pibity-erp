...
import { DiffRow, MappingProductTagRow } from '../../../main/rows'
import { MappingProductTagVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.MappingProductTag.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<MappingProductTagVariable>>().appendAll(rows ? rows.map(x => MappingProductTagRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/mapping-product-tag') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
