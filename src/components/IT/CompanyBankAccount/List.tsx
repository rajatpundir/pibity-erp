...
import { DiffRow, CompanyBankAccountRow } from '../../../main/rows'
import { CompanyBankAccountVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.CompanyBankAccount.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<CompanyBankAccountVariable>>().appendAll(rows ? rows.map(x => CompanyBankAccountRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/company-bank-account') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
