...
import { DiffRow, ContactAddressRow } from '../../../main/rows'
import { ContactAddressVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ContactAddress.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ContactAddressVariable>>().appendAll(rows ? rows.map(x => ContactAddressRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/contact-address') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
