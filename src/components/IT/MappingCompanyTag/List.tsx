...
import { DiffRow, MappingCompanyTagRow } from '../../../main/rows'
import { MappingCompanyTagVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.MappingCompanyTag.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<MappingCompanyTagVariable>>().appendAll(rows ? rows.map(x => MappingCompanyTagRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/mapping-company-tag') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
