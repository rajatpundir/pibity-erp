...
import { DiffRow, ProductCategoryRow } from '../../../main/rows'
import { ProductCategoryVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.ProductCategory.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ProductCategoryVariable>>().appendAll(rows ? rows.map(x => ProductCategoryRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/product-category') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
