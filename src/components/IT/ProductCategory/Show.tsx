...
export type Action =
    ...
    | ['variable', 'parent', ProductCategory]
    | ['variable', 'group', ProductCategoryGroup]
    | ['variable', 'name', string]
    | ['variable', 'code', string]
    | ['variable', 'derivedCode', string]
    | ['variable', 'childCount', number]
    ...
    | ['replace', 'variable', ProductCategoryVariable]
...
function Component(props) {
    ...
    const setVariable = useCallback(async () => {
        if (props.match.params[0]) {
            const rows = await db.ProductCategory.toArray()
            var composedVariables = HashSet.of<Immutable<ProductCategoryVariable>>().addAll(rows ? rows.map(x => ProductCategoryRow.toVariable(x)) : [])
            ...
            if (variables.length() === 1) {
                ...
                dispatch(['replace', 'variable', variable as ProductCategoryVariable])
                ...
                dispatch(['replace', 'productCategoryList', composedProductCategoryVariables.filter(variable => variable.values.parent.hashCode() === props.match.params[0]) as HashSet<ProductCategoryVariable>])
            }
        }
    ...
    const createVariable = async () => {
        const [result, symbolFlag, diff] = await executeCircuit(circuits.createProductCategory, {
            ...
            derivedCode: state.variable.values.derivedCode,
            childCount: state.variable.values.childCount,
    ...
    const deleteVariable = async () => {
        const [result, symbolFlag, diff] = await executeCircuit(circuits.deleteProductCategory, {
            ...
            return <Container area={none} layout={Grid.layouts.main}>
                ...
                <Item area={Grid.button} justify='end' align='center' className='flex'>
                    {
                        iff(state.mode === 'create',
                            <Button onClick={async () => {
                                ...
                                props.history.push('/product-category-list')
                            }}>Save</Button>,
                ...
                <Container area={Grid.details} layout={Grid.layouts.details}>
                        ...
                        <Label>{productCategoryType.keys.parent}</Label>
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
                                ...
                                <div className='font-bold text-xl'>{
                                    iff(productCategoryList.filter(x => x.id.hashCode() === state.variable.values.parent.hashCode()).length() !== 0,
                                        () => {
                                            const referencedVariable = productCategoryList.filter(x => x.id.hashCode() === state.variable.values.parent.hashCode()).toArray()[0] as ProductCategoryVariable
                                            return <Link to={`/product-category/${referencedVariable.id.hashCode()}`}>{referencedVariable.id.hashCode()}</Link>
                                        }, <Link to={`/product-category/${state.variable.values.parent.hashCode()}`}>{state.variable.values.parent.hashCode()}</Link>)
                                }</div>
                            )
                        }
                    </Item>
                    <Item>
                        <Label>{productCategoryType.keys.group}</Label>
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
                                ...
                                <div className='font-bold text-xl'>{
                                    iff(productCategoryGroupList.filter(x => x.id.hashCode() === state.variable.values.group.hashCode()).length() !== 0,
                                        () => {
                                            const referencedVariable = productCategoryGroupList.filter(x => x.id.hashCode() === state.variable.values.group.hashCode()).toArray()[0] as ProductCategoryGroupVariable
                                            return <Link to={`/product-category-group/${referencedVariable.id.hashCode()}`}>{referencedVariable.id.hashCode()}</Link>
                                        }, <Link to={`/product-category-group/${state.variable.values.group.hashCode()}`}>{state.variable.values.group.hashCode()}</Link>)
                                }</div>
                            )
                        }
                    </Item>
                    ...
                    <Item>
                        ...
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
                                <div className='font-bold text-xl'>{state.variable.values.derivedCode}</div>,
                                <div className='font-bold text-xl'>{state.variable.values.derivedCode}</div>
                            )
                        }
                    </Item>
                    <Item>
                        ...
                        {
                            iff(state.mode === 'create' || state.mode === 'update',
                                <Input type='number' onChange={onVariableInputChange} value={state.variable.values.childCount} name='childCount' />,
                                <div className='font-bold text-xl'>{state.variable.values.childCount}</div>
                            )
                        }
                    </Item>
                </Container>
