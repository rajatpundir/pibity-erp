...
import { DiffRow, ContactRow } from '../../../main/rows'
import { ContactVariable } from '../../../main/variables'
...
const initialState: State = {
    typeName: 'Contact',
    query: getQuery('Contact'),
    limit: 5,
    offset: 0,
    page: 1,
    columns: Vector.of(['values', 'name'], ['values', 'email'], ['values', 'telephone'], ['values', 'mobile'], ['values', 'website'])
}
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.Contact.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<ContactVariable>>().appendAll(rows ? rows.map(x => ContactRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/contact') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
