...
import { DiffRow, PurchaseInvoiceItemRow } from '../../../main/rows'
import { PurchaseInvoiceItemVariable } from '../../../main/variables'
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.PurchaseInvoiceItem.orderBy('id').toArray())
    var composedVariables = Vector.of<Immutable<PurchaseInvoiceItemVariable>>().appendAll(rows ? rows.map(x => PurchaseInvoiceItemRow.toVariable(x)) : [])
    ...
    return (
        <Container area={none} layout={Grid.layouts.main} className='p-10'>
            <Item area={Grid.header} align='center' className='flex'>
                ...
                <button onClick={() => { props.history.push('/purchase-invoice-item') }} className='text-3xl font-bold text-white bg-gray-800 rounded-md px-2'>+</button>
            </Item>
