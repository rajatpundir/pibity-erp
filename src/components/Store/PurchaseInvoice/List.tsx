...
const initialState: State = {
    ...
    offset: 0,
    page: 1,
    columns: Vector.of(['variableName'], ['values', 'purchaseOrder'])
...
function Component(props) {
    ...
    const rows = useLiveQuery(() => db.purchaseInvoices.orderBy('variableName').toArray())
    var composedVariables = Vector.of<Immutable<PurchaseInvoiceVariable>>().appendAll(rows ? rows.map(x => PurchaseInvoiceRow.toVariable(x)) : [])
